...
async def notify_pending(session) -> int:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for e in events:
            r=await client.post(f"https://api.telegram.org/bot{settings.telegram_bot_token}/sendMessage", json={"chat_id":settings.telegram_chat_id,"text":format_event(e),"parse_mode":"HTML","disable_web_page_preview":True})
